...
- 总览页新增“多源证据层”：联动展示目标年P2来源、BIS选定规则的人工范围核对结果，以及IMF CDIS双向投资头寸；后三者各自保留年份和范围，不把非同口径资料并入主分数。
- 新增“来源覆盖账本”：随年份切换，展示Comtrade、UNGA、TiVA、P2在12国样本中的实际覆盖；另列BIS复核范围、CDIS双方向缺报以及暂未接入的GDELT、GTA/CSL。
- GDELT 仅作中美关系背景参照；GTA 与美国综合出口管制清单列为企业/商品级补充候选。来源粒度、连接键、文件哈希和未接入原因见 [`data/数据来源登记表.csv`](data/数据来源登记表.csv) 与 [`docs/多源数据整合与连接审计.md`](docs/多源数据整合与连接审计.md)。
